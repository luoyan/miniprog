...
public interface PageStatsDAO {
    ...
    public List<PageStatsData> getAllBetweenDateByTypeAndDimensions(Date startDate, Date endDate, String TypeId, String Dimensions);

    @SQL("SELECT " + ALL_FIELDS + " FROM " + TABLE + " WHERE Date >= :1 and Date <= :2 and TypeId in (:3) and Dimension in (:4) order by Date")
    public List<PageStatsData> getAllBetweenDateByTypesAndDimensions(Date startDate, Date endDate, List<String> TypeIdList, List<String> DimensionList);
    ...
    @SQL("select Date from " + TABLE + " WHERE TypeId in (:1) and Dimension in (:2) order by Date desc limit 1;")
    public Date getRecentDate(List<String> TypeIdList, List<String> DimensionList);
}
